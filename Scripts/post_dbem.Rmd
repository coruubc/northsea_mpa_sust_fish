...
World_map <- rnaturalearth::ne_countries(scale = 'large', returnclass = c("sf"))
  ...
  ggplot() +
    ...
    geom_tile(data = subset(test, !is.na(bins)),
    aes(
      x = lon,
      y = lat,
      fill = bins,
  ...
  geom_tile(data = subset(grids, status == "protected"),
    aes(
      x = lon,
      y = lat
    ),
    ...
    scale_fill_viridis("Proportion increase", discrete = T) + 
    scale_color_viridis("Proportion increase",discrete = T) + 
    theme_bw() +
...
grid_mpa <- read_csv("Data/mpa_scenarios/grid_m5point5.csv") %>% 
  select(index = gridID, zone = zone.x,status,prop)


load("Data/dbem_outputs/C6GFDL85F075MPA05/600032Catch.RData")

abd <- as.data.frame(sppabdfnl) 
colnames(abd) <- seq(1951,2100,1)

abd <- abd %>% 
  bind_cols(Lon_Lat_DBEM) %>% 
