...
```
## Option two, one scenario per plot
...
grids <- read_csv(here("./Data/mpa_scenarios/grid_m5point5.csv")) %>% 
  ...
  mutate(status = replace_na(status,"no protected"))
...
# pal <- "IsleofDogs1"
  ...
  aes(
    ...
    # scale_fill_manual("Conservation status",values = c(wes_palette(pal)[1],
    #                              wes_palette(pal)[2],
    #                              wes_palette(pal)[3]
    #                              )
    #                   ) +
    # scale_color_manual("Conservation status",values = c(wes_palette(pal)[1],
    #                              wes_palette(pal)[2],
    #                              wes_palette(pal)[3]
    #                              )
    #                   ) +
    scale_fill_viridis("Conservation status", discrete = T, alpha = 0.5, option = "D",direction = -1) +
    scale_color_viridis("Conservation status",discrete = T, alpha = 0.5, option = "D",direction = -1) +
    theme_bw() +
    labs(x = "Longitude", y = "Latitude") +
    theme(legend.position = "bottom") +
    facet_grid(status~cons_scen) +
    coord_sf(xlim = c(-42.5,69),
             ylim = c(35,90)
             ) +
    ggsave("scenarios_b.png",
           width = 8,
           height = 8
           )
```
